  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8 space-y-10">
      <section className="flex items-center justify-between">
        ...
        <button onClick={handleLogout} className="bg-[#f85606] text-white px-4 py-2 rounded hover:bg-[#e14e00]">Logout</button>
      </section>

      <section>
        <h2 className="text-2xl font-semibold mb-4">Wishlist</h2>
        {wishlist.length === 0 ? (
          <p className="text-gray-600 dark:text-gray-300">No items in wishlist yet.</p>
        ) : (
      ...
      <section>
        ...
        {orders.length === 0 ? (
          ...
          <div className="space-y-4">
            {orders.map((o, idx) => (
              <div key={idx} className="border rounded-lg p-4 shadow bg-white dark:bg-gray-900 dark:text-white">
                ...
                <div className="text-sm text-gray-700 dark:text-gray-300">Items: {o.items.reduce((s, it) => s + it.quantity, 0)}</div>
                <div className="text-orange-600 font-bold mt-1">Total: ${o.total.toFixed(2)}</div>
              </div>
            ))}
          </div>
        )}
