...
export async function PATCH(
  ...
  try {
    ...
    if (refundMethod)
      updateData.refundMethod = refundMethod as "original" | "store-credit";
    ...
    if (oldStatus !== status) {
      try {
        const updatedReturnRequest =
          await returnService.getReturnRequestById(returnId);
        if (updatedReturnRequest) {
          ...
          if (emailType && order) {
            await sendReturnRequestEmail(
              ...
              order,
              emailType as "approved" | "rejected" | "completed" | "refunded",
            );
          }
        }
      } catch (emailError) {
