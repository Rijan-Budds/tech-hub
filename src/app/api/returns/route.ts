...
export async function GET() {
  try {
    ...
    const returnRequests = await returnService.getReturnRequestsByUserId(auth.sub);
    return NextResponse.json({ returnRequests });
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    // Validate reason
    const validReasons = ['damaged', 'wrong-item', 'size-issue', 'defective', 'not-as-described', 'other'];
    if (!validReasons.includes(reason)) {
      return NextResponse.json({ message: 'Invalid return reason' }, { status: 400 });
    }
    ...
    if (order.userId !== auth.sub) {
      return NextResponse.json({ message: 'Unauthorized - Order does not belong to user' }, { status: 403 });
    }
    
    // Check if order is eligible for return
    if (!returnService.isOrderEligibleForReturn(order)) {
      return NextResponse.json({ 
        message: 'Order is not eligible for return. Orders can only be returned within 7 days of delivery.' 
      }, { status: 400 });
    }
    
    // Check if return request already exists for this order
    const existingReturnRequest = await returnService.getReturnRequestByOrderId(orderId);
    ...
    const returnRequestId = await returnService.createReturnRequest(returnRequestData);
    ...
    console.log("POST /api/returns - Return request created with ID:", returnRequestId);
    
    // Send return request submitted email
    try {
      const createdReturnRequest = await returnService.getReturnRequestById(returnRequestId);
    ...
    const updatedReturnRequests = await returnService.getReturnRequestsByUserId(auth.sub);
