...
export default function OrderConfirmationPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const found =
          (data.orders || []).find(
            (o: Order) => o.id === orderId || o._id === orderId,
          ) || null;
        setOrder(found);
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      ...
      case "canceled":
        return { color: "text-red-600", bg: "bg-red-100", text: "Canceled" };
      default:
        return { color: "text-gray-600", bg: "bg-gray-100", text: status };
    }
  };
        ...
        <div className="max-w-3xl mx-auto px-6 py-12">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              You need to log in to view your order.
            </p>
            <button
              onClick={() => router.push("/")}
      ...
      <div className="max-w-3xl mx-auto px-6 py-12 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 space-y-4 border border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center justify-between">
            ...
            <div
              className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusInfo(order.status).bg} ${getStatusInfo(order.status).color}`}
            >
        ...
        {returnRequest && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 space-y-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              ...
              <div
                className={`px-3 py-1 rounded-full text-sm font-medium ${getReturnStatusInfo(returnRequest.status).bg} ${getReturnStatusInfo(returnRequest.status).color}`}
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              {returnRequest.processedAt && (
                <div>
                  ...
                  <span className="ml-2 font-medium">
                    {new Date(returnRequest.processedAt).toLocaleDateString()}
                  </span>
                </div>
              )}
            </div>
            ...
            {returnRequest.refundAmount && (
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg p-3">
                <p className="text-sm text-green-800 dark:text-green-300">
                  <strong>Refund Amount:</strong> रु
