...
export const productService = {
  ...
  async getProductsByCategoryWithPagination(
    category: string,
    page: number,
    limit: number,
    sortBy: string = "createdAt",
...
export const orderService = {
  // Create a new order
  async createOrder(
    orderData: Omit<IOrder, "id" | "createdAt">,
  ): Promise<string> {
...
export const returnService = {
  ...
  async createReturnRequest(
    returnData: Omit<IReturnRequest, "id" | "requestedAt">,
  ): Promise<string> {
  ...
  async getReturnRequestById(returnId: string): Promise<IReturnRequest | null> {
    const returnDoc = await getDoc(
      doc(db, COLLECTIONS.RETURN_REQUESTS, returnId),
    );
    if (returnDoc.exists()) {
      ...
      return {
        ...
        processedAt: data.processedAt
          ? timestampToDate(data.processedAt)
          : undefined,
      } as IReturnRequest;
    }
    return null;
  ...
  async updateReturnRequest(
    returnId: string,
    updates: Partial<IReturnRequest>,
  ): Promise<void> {
  ...
  isOrderEligibleForReturn(order: IOrder): boolean {
    ...
    if (
      order.deliveredAt &&
      (order.deliveredAt instanceof Timestamp ||
...
export const reviewService = {
  // Create a new review
  async createReview(
    reviewData: Omit<IReview, "id" | "createdAt" | "updatedAt">,
  ): Promise<string> {
  ...
  async getReviewsByProductIdWithPagination(
    productId: string,
    page: number,
    limit: number,
    sortBy: string = "createdAt",
    ...
    try {
      ...
      const productReviews = await this.getReviewsByProductId(productId);
      const totalCount = productReviews.length;

      // Sort reviews
      const sortedReviews = productReviews.sort((a, b) => {
        let aValue: unknown = a[sortBy as keyof IReview];
        let bValue: unknown = b[sortBy as keyof IReview];
      ...
      console.error("Error in getReviewsByProductIdWithPagination:", error);
      return {
        reviews: [],
        totalCount: 0,
      };
  ...
  async getAllReviewsWithPagination(
    page: number,
    limit: number,
    sortBy: string = "createdAt",
    sortOrder: string = "desc",
    ...
    try {
      ...
      console.error("Error in getAllReviewsWithPagination:", error);
  ...
  async updateReview(
    ...
    if (updates.rating && updates.productId) {
      await this.updateProductRatingStats(updates.productId);
    }
  },

  // Delete review
  async deleteReview(reviewId: string): Promise<void> {
    // Get the review first to know which product to update
